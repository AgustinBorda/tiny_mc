...
static void photon(void)
{
	...
	/* #################  LOOP SIN ROULETTE  ####################*/
	for(unsigned int j=0;j<n_roulette;j++){ // Iterar sobre los 73 pasos SIN ROULETTE
	...
	for(unsigned int j=0;j<n_roulette;j++){ //La energia que se deposita depende del paso j
